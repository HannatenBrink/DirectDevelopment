...
#define DYTOL               	1.0E-5//3//5          // Variable tolerance (5,5,9 used for the last ERP)
#define RHSTOL              	1.0E-5//3//5     		// Function tolerance
#define ESSTOL              	1.0E-9//4//9
...
#define X0          parameter[15]	// Size at birth Default:
#define XMIN        parameter[16]   // Size where R2 is available
#define XJ          parameter[17]   // Size at metamorphosis
#define XF          parameter[18]	// Size at maturity Default:

#define QJ          parameter[19]	// Default:
#define QA          parameter[20]	// Default:
#define QS          parameter[21]	// Starvation threshold. Default:
                 ...
                 double development[POPULATION_NR][I_STATE_DIM])
{
    ...
    PSI_A = min(1.0, PSI + META);                       //Specialization R1 J&A
    w = (1.0+QJ)*IR;                                  // Effective body mass
    wmin = (1.0+QJ)*XMIN;                               // Min Weight for R2 (nu wanneer larval stage is beendig)
    ...
    PHI = 1.0 / (1.0 + pw);                                  //Time on R1
    gamma = (a1*PHI*R1+a2*(1-PHI)*R2) / (1.0 + h*(a1*PHI*R1+a2*(1.0 - PHI)*R2));      //Intake rate
    In1 =(a1*PHI*R1) / (1.0+ h*(a1*PHI*R1+a2*(1.0 - PHI)*R2));                           //Intake rate R1
    In2 = (a2*(1.0 - PHI)*R2) / (1.0+ h*(a1*PHI*R1+a2*(1.0 - PHI)*R2));                      //Intake rate R2
    Em = M1*pow(IR+REV, M2);                                                                    //maintenance cost
    Ei = (K1)*gamma;                                                                              //Conversion efficiency
    Eg = Ei - Em;                                                                               // Net production
    kappa_qj = 1.0/ ((1.0+QJ)*QJ) * (REV / IR);                                                      //fraction allocated to irreversible mass juve
    kappa_qa = 1.0/ ((1.0+QA)*QA) * (REV / IR);
    ...
    {
        double fecfrac = 1.0 -(1+QJ)*QJ/((1.0 +QA)*QA);    //1-(ka/kj)
        ...
        development[0][2] = (1 - kappa_qa - fecfrac)*Eg;  	//rev  rev naar adults = revJ * kappa_A/kappa_J = (1-kappa_J)*kappa_A/kappa_J -> dan heb je zelfde functie maar andere verhouding.
    }
...
void DiscreteChanges(int lifestage[POPULATION_NR], double *istate[POPULATION_NR],
                     double *birthstate[POPULATION_NR], int BirthStateNr, double E[])
{
    if ((lifestage[0] == 1) &&   ((XJ-X0) > 0)) //XJ not too small CHECK IF THIS IS CORRECT IN CASE meta is before birth
    {
        REV = REV - META * (QJ - QS) * max(0,(XJ - X0));
        SetSurvival(0,(1.0 - META*Rho)*exp(istate[0][I_STATE_DIM]));
    }
               ...
               double *fecundity[POPULATION_NR])
{
    ...
    else
    {
        double surv;
        double eggenergy;
        double fecfrac = 1.0 - (1.0+QJ)*QJ/((1.0+QA)*QA); //
        double s = (1000 * (REV / (QJ * IR)) - 999);   ///stepfunctie betweeb 0 and 1 (0 if rev<qj*x, 1 if rev=>qj*x)
        s = (s < 0) ? 0 : s;
        s = (s > 1) ? 1 : s;
        fecfrac *= 3.0*s*s - 2.0*s*s*s;  ///sigmoid
        if ((XJ-X0) <= 0) {
            surv = min(1.0,(1.0 - META*Rho)); //Prob to survive Meta
            eggenergy = (1.0 + QJ)*X0; //Cost of an egg
        }
        else {
            surv = 1.0;
            eggenergy =(1.0 + QJ)*X0;
            ...
            double impact[POPULATION_NR][INTERACT_DIM])
{
    ...
    PHI = 1.0 / (1.0 + pw);                                     //Time on R1
    gamma = (a1*PHI*R1+a2*(1.0 -PHI)*R2) / (1.0 + h*(a1*PHI*R1+a2*(1.0-PHI)*R2));      //Intake rate
    In1 =(a1*PHI*R1) / (1.0 + h*(a1*PHI*R1+a2*(1.0-PHI)*R2));                           //Intake rate R1
    In2 = (a2*(1-PHI)*R2) / (1.0 + h*(a1*PHI*R1+a2*(1.0-PHI)*R2));                      //Intake rate R2
    Em = M1*pow(IR+REV, M2);                                                                    //maintenance cost
    Ei = (K1)*gamma;                                                                              //Conversion efficiency
    Eg = Ei - Em;                                                                               // Net production
    kappa_qj = 1.0 / ((1.0+QJ)*QJ) * (REV / IR);                                                      //fraction allocated to irreversible mass juve
    ...
    double	maxage = 1.0, birthnul = 1.0;
    if (Birthrate(0) < 1E-9) {
        maxage = 0.0;
        birthnul = 0.0;
    } else {
        maxage = 1.0 / (Birthrate(0) * Survival(0));
        birthnul = 1.0 / Birthrate(0);
    }
